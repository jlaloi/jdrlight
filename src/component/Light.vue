...
import { DELETE_LIGHT, UPDATE_LIGHT, ALL_SCENES } from '../config/graph.js';
...
export default {
  ...
  methods: {
    ...
    update(
      ...
      const findScene = s => s.id === this.light.scene.id;
      const findLightIndex = l => l.id == this.light.id;
      const lightIndex = data.allScenes
        .find(findScene)
        .lights.findIndex(findLightIndex);
      data.allScenes.find(findScene).lights[lightIndex] = updateLight;
    ...
    updateDelete(
      ...
      const data = proxy.readQuery({ query: ALL_SCENES });
      const findScene = s => s.id === this.light.scene.id;
      data.allScenes.find(findScene).lights = data.allScenes
        .find(findScene)
        .lights.filter(l => l.id !== deleteLight.id);
      proxy.writeQuery({ query: ALL_SCENES, data });
    },
    reset() {
      this.power = this.light.power;
